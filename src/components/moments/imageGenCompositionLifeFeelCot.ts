...
【与上方自拍协议的关系】
- **[wx-selfie|who=…] 自拍或对镜**：只写 \`selfie shot\` / \`mirror selfie shot\` + upper body/穿搭/环境；**禁止**默认 close-up / 怼脸；仅角色明确要特写时才写 close-up 部位。
- **非自拍·直接画面、半身以上生活场景**：须严格执行下列规则。
...
2. **构图**：默认半身 / 三分之二身 / 坐姿中景 / 带动作站姿 / 过肩 / 斜侧；禁大头照、证件照、无背景近景、纯正面站桩（自拍协议除外）。**双人亲密同框**须 medium/wide，或只写被拍方画面 + own hand/forearm 从画缘入镜，禁单人 close-up。
