...
export function pickOrientationFieldExample(prefs: MeetOrientationPreference[]): string {
  if (!prefs.length) return '双性恋 / 泛性恋'
  const primary = prefs[0]!
  const samples: Record<MeetOrientationPreference, string> = {
    hetero: '异性恋 (Hetero)',
    homo: '同性恋 (Homo)',
    bi_pan: '双性恋 / 泛性恋',
    ace: '无性恋谱系',
  ...
  return samples[primary] ?? '双性恋'
}

/** 用户勾选异性恋且限定目标性别时，提示模型对齐性别与取向 */
export function buildGenderOrientationAlignmentHint(filters: RadarFilters): string {
  ...
  if (!prefs.length || !prefs.includes('hetero')) return ''
  if (prefs.includes('homo') && prefs.length > 1) return ''

  if (filters.gender === 'male') {
    return '【性别·取向对齐】用户寻觅目标性别为**男**，且接受异性恋设定：生成角色 gender 应为**女**，顶层 orientation 须明确为**异性恋**（勿写成同性恋）。'
  }
  if (filters.gender === 'female') {
    return '【性别·取向对齐】用户寻觅目标性别为**女**，且接受异性恋设定：生成角色 gender 应为**男**，顶层 orientation 须明确为**异性恋**（勿写成同性恋）。'
  }
  return '【取向对齐】用户接受异性恋设定：顶层 orientation 字段须明确写出「异性恋」，comprehensive.psyche.orientationOrigin 与之一致，**禁止**默认写成同性恋。'
}

/** 注入 AI：比旧版更硬的取向约束 */
export function buildEncounterOrientationCriteriaBlock(filters: RadarFilters): string {
  const prefs = filters.orientationPreferences
  if (!prefs.length) {
  ...
  const example = pickOrientationFieldExample(prefs)
  ...
  if (onlyHetero) {
    ...
    lines.push('用户**未**勾选同性恋：不要把 orientation 默认写成同性恋；须从用户已勾选类型中择一写实。')
  }
