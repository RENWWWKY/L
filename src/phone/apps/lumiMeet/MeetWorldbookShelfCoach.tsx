...
import {
  MEET_WB_SHELF_COACH_STEPS,
  meetWbCoachTargetSelector,
  type MeetWbShelfCoachStep,
  type MeetWbShelfCoachTargetId,
} from './meetWorldbookShelfCoachSteps'
...
function measureTargetInOverlay(target: MeetWbShelfCoachTargetId, overlayEl: HTMLElement): HoleRect | null {
  const node = document.querySelector(meetWbCoachTargetSelector(target))
  if (!node) return null
...
export type MeetWorldbookShelfCoachProps = {
  open: boolean
  stepIndex: number
...
function CoachCardBody({
  ...
  return (
    <>
      ...
      <p id="meet-wb-coach-step-title" className="mt-2 text-[15px] font-medium tracking-[0.06em] text-[#2c2a26]">
        {step.title}
      </p>
...
export function MeetWorldbookShelfCoachPortal({
  open,
  stepIndex,
  onStepChange,
  onSkip,
  onComplete,
}: MeetWorldbookShelfCoachProps) {
  const portalEl = getLumiMeetPortalTarget()
  const overlayRef = useRef<HTMLDivElement>(null)
  const step = MEET_WB_SHELF_COACH_STEPS[stepIndex]
  const total = MEET_WB_SHELF_COACH_STEPS.length
  ...
  const remeasure = useCallback(() => {
    ...
    if (step.target) {
      document.querySelector(meetWbCoachTargetSelector(step.target))?.scrollIntoView({
        block: 'nearest',
        inline: 'nearest',
        behavior: 'instant',
      })
  ...
  return createPortal(
    <AnimatePresence>
      {open ? (
        <motion.div
          ref={overlayRef}
          key="meet-wb-shelf-coach"
          role="dialog"
          aria-modal="true"
          aria-labelledby="meet-wb-coach-step-title"
          className="fixed inset-0 z-[395] overflow-hidden"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        >
