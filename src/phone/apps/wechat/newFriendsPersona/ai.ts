...
/** 模型失败时的网名池：禁止用角色「姓名」字段直出，只用网感碎片词 */
const WECHAT_NICK_FALLBACK_POOL = [
  '摸鱼中',
  '睡不醒',
  '随便吧',
  '不聊了',
...
/** 绝不使用角色档案里的「姓名」当微信昵称 */
function fallbackWechatNicknameFromCharacter(c: Character): string {
  const prev = (c.wechatNickname || '').trim()
  if (prev && !nicknameIsLegalNameClone(prev, c) && isPlausibleWechatNickname(prev)) {
    return clampNick(prev)
  }
  const idx = hashSeed(c.id) % WECHAT_NICK_FALLBACK_POOL.length
  return clampNick(WECHAT_NICK_FALLBACK_POOL[idx])
}
...
- 【性质】这是挂在个人资料上、对所有好友可见的「自我状态/态度碎片」，像随手改的一句心情或吐槽；**不是**对某一个具体人说话，**不是**对屏幕前用户/读者的叮嘱、说教、关心喊话。
- 【参考座右铭风格】整体气质可向“座右铭”靠拢：短、稳、准、有人设立场；像角色会认同的一句自我准则。
- 【禁止】第二人称（你/您）与任何「让读者觉得在被管」的文案：如「别抽了」「对身体不好」「早睡」「多喝水」「照顾好自己」「要开心」「听话」等——哪怕符合人设习惯，也必须改成**第一人称或无人称**的自我表达（例：人设会劝人戒烟 → 签名用「戒了三天」或「烦烟味」，不要用「别抽了」）。
- 【禁止】私聊口吻、对话体、情侣留言体（想你/等你/么么等）。
- 以 4～15 个字为主，最多不超过 20 个字；优先 8～12 字的凝练短句，可极简到「。」「嗯」「算了」。
- 【去人机感】优先生成“像本人刚改完”的状态碎片：允许口语、省略、停顿、轻微毛边，不要四平八稳的模板句。
- 【禁抄硬规则】用户给的参考签名只能借风格气质，绝对禁止照搬原句；也禁止同义改写后抄结构。
- 【原创硬规则】禁止输出互联网上高频金句、鸡汤、爱情宣言、价值口号；每条都要像该角色专属写法。
- 【语义要求】除“状态感”外，必须带一点角色的价值取向/边界感/做事方式，读起来像一句“人设态度句”。
- 【风格路由】按角色人设自动选择主风格并收敛语气：
  1) 高冷疏离：边界感强、短促克制、少解释。
  2) 温柔治愈：轻柔有生活感，但不鸡汤不过度文艺。
  3) 目标感男生：行动导向、简洁有劲、少空话。
  4) 独立女生：自我主语、清醒自持、不过度喊口号。
  5) 抽象搞笑：轻自嘲、微发疯、可读不低俗。
  6) 极简英文：仅当角色确有英文习惯时可用；最多 3 个英文词，不要整句鸡汤英文。
- 【风格示例（只借气质，不可照搬原句）】
  - 高冷疏离参考：你的看法，与我无关。/ 圈子不同，别硬融。/ 社交降级，快乐升级。
  - 温柔治愈参考：慢慢来，好戏都在烟火里。/ 去爱那些让你觉得世界可爱的人和事。/ 愿每一阵风都带着好消息。
  - 目标感男生参考：少说多做，实力说话。/ 沉淀自己，默默发光。/ 拿得起，放得下，干脆利落。
  - 独立女生参考：知足且上进，温柔而坚定。/ 成为自己的月亮，不用借谁的光。/ 做不被定义的风，去哪都自由。
  - 抽象搞笑参考：退堂鼓国家级表演艺术家。/ 脆皮打工人，易碎请轻放。/ 每天都在“算了吧”和“凭什么”之间反复横跳。
  - 极简英文参考：Stay real or stay away. / Serendipity. / Love yourself first.
- 【严格去重】以上示例不能原句输出，不能只改 1~2 个字；必须换表达、换结构、换意象后再写。
- 【结构偏好】采用“座右铭感 + 状态快照”混合：一句话里既有立场，也有当下感；拒绝空洞大词。
...
export async function generateWechatProfilesForPersonaCharacters(params: {
  ...
  for (const it of rows) {
    ...
    let nickname = typeof o.nickname === 'string' ? clampNick(o.nickname) : ''
    if (ch && nicknameIsLegalNameClone(nickname, ch)) nickname = fallbackWechatNicknameFromCharacter(ch)
    if (!isPlausibleWechatNickname(nickname)) nickname = ch ? fallbackWechatNicknameFromCharacter(ch) : clampNick('摸鱼中')
  ...
  for (const c of list) {
    ...
    let nick = (c.wechatNickname || '').trim()
    if (!nick || nicknameIsLegalNameClone(nick, c)) nick = fallbackWechatNicknameFromCharacter(c)
    else nick = clampNick(nick)
    out.push({
      characterId: c.id,
      wechatNickname: nick,
      wechatSignature: clampSig((c.wechatSignature || '在。').trim()),
      wechatId: normalizeWechatId((c.wechatId || '').trim(), c.id),
      motto: clampMotto((c.motto || '').trim()),
    })
  }
  return out
}
