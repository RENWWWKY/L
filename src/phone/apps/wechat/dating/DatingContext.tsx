...
async function generateDatingAi(
  ...
  if (!apiConfig?.apiUrl || !apiConfig?.apiKey || !apiConfig?.modelId) {
    ...
    return `<thinking>
【Lumi大人之书】占位续写；承接玩家意图与人设边界。本分册·必查：是。
【时间与环境之书】当场时间/地点一笔。本分册·必查：无瞬移：是。
【互动轴之书】意图摘要一句（非复读原文）。本分册·必查：是。
【认知与逻辑之书】仅写角色可知情点。本分册·必查：无私聊外挂：是。
【关系梯度之书】阶段一句；数值仅场记。本分册·必查：正文无数值：是。
【文风与禁词之书】拟用首句类型：对白起笔；非比喻。本分册·必查：是。
【推进与钩子之书】锚点+衔接；动作→连锁；**我…** OS 至少1句。本分册·必查：是。
【防止抢话】与本轮模式一致。本分册·必查：无抢话：是。
【Lumi预稿自检】预检维度1～12：占位均「该项：无」
自检结论：通过
</thinking>
...
"${seed.slice(0, 24)}。"他低声接住这个话题，语气平稳。`
  ...
  const roleMode = godPerspective
    ? '上帝视角：只写用户当前看不见、也不知晓的非面对面角色/NPC场景；**旁白一律第三人称写约会对象与 NPC，「你」仅指玩家**（须在思维链【防止抢话】与预检维度 8 中闭环）；禁止描写用户当下可见现场，禁止与用户直接对话；**与抢话互斥，不得代写玩家当轮言行**。**不得把「线上近期聊天」里已出现的气泡内容，改写成旁白里又发给用户/又讲一遍同款行程**；须写屏幕外或未写过的信息。'
  ...
  const lengthRule =
    ...
    `【思维链·速度】\`<thinking>\` 内全文建议 **≤ 900 汉字**（含【】标题）；各分册各 **1～3 句** 即可；【Lumi预稿自检】12 项可 **每项一行**（「无」须带半句理由）。**禁止**在思维链里写数千字长文——会极慢且易超出接口上限。`
  ...
  const onlinePrivBoundaryReminder =
    onlineRecentClipped.length > 8
      ? `【当轮强提醒·知悉边界】下列摘录多为**私聊**；线下**其他 NPC** 不得无因知晓用户与**${character.realName}**私聊的具体内容；**${character.realName}**也不得无因知晓用户与其他 NPC 私聊的内容，除非摘录或前文已给出合法知情路径（须在思维链【认知与逻辑之书】与预检 12 中自检）。\n`
      : ''
  const wechatDialogueParityReminder =
    onlineRecentClipped.length > 8
      ? `【当轮强提醒·对白口吻】下方「线上近期聊天」为**同一 ${character.realName}** 的微信样本：**对白**须与摘录口吻一致——口语、短句、活人感；场景是面对面，**不是**换个人写小说腔长台词；**勿**在引号对白里堆「（笑）」类括号神态（须在思维链【文风与禁词之书】/预检 4 中闭环）。\n`
      : ''
